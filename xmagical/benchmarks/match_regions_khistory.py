...
class MatchRegionsKHistoryEnv(BaseEnvXirl):
    ...
    def __init__(
        ...
        self._state_history = deque(maxlen=k_history)
        if rand_shapes:
        ...
        if self.use_state:
            ...
            state_dim = c + 4 * max_num_debris  # or len(get_state()) if dynamically available
            obs_low = np.array([-1.0] * (self.k_history * state_dim), dtype=np.float32)
            obs_high = np.array([+1.0] * (self.k_history * state_dim), dtype=np.float32)
    ...
    def on_reset_default(self):
        ...
        if self.use_state:
            ...
            state_dim = c + 4 * len(self.__debris_shapes)  # or len(get_state()) if dynamically available
            obs_low = np.array([-1.0] * (self.k_history * state_dim), dtype=np.float32)
            obs_high = np.array([+1.0] * (self.k_history * state_dim), dtype=np.float32)
...
###################### NEW STUFF ######################
    ...
    def _get_stack_state(self) -> np.ndarray:
        return np.concatenate(list(self._state_history), axis=0)
    ...
    def reset(self) -> np.ndarray:
        ...
        if self.use_state:
            ...
            for _ in range(self.k_history):
                self._state_history.append(first_state)
